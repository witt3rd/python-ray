...
DEFAULT_RADIUS = 1.0
DEFAULT_BOUNDS = (-DEFAULT_RADIUS, -DEFAULT_RADIUS,
...
def make_update(k, N, counter_instance,
    ...
    def update(counter):
        ...
        pi, count_inside, count_total, xys_in, xys_out = pi_calc.sample(
            n_per_pi_calc)
...
def make_dmaps(Ns=DEFAULT_NS):
    ...
    for k in range(len(Ns)):
        ...
        dmap = hv.DynamicMap(dmap_update, streams=[counter]).opts(
            height=psize, width=psize)

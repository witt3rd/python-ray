...
class MonteCarloPi():
    ...
    def sample(self, num_samples):
        ...
        _, in_circle, xys_in, xys_out = estimate_pi(
            num_samples, return_points=True)
...
# We'll do this many calculations for a given N and average the results.
repeat = 10
...
def ray_compute_pi_loop(N):
    ...
    # only return the id for Pi values
    pi_ids = [actor.sample.remote(N)[0] for actor in actors]
...
def main():
    ...
    print("\nResults without Ray:")
    ns, means, stddevs, errors, durations = compute_pi_for(
        args.Ns, compute_pi_loop)
    ...
    ray_ns, ray_means, ray_stddevs, ray_errors, ray_durations = compute_pi_for(
        args.Ns, ray_compute_pi_loop)

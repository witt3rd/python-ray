...
def square_circle_plot(radius=1.0, title=''):
    ...
    plot.arc(x=0.0, y=0.0, radius=radius, start_angle=0.0,
             end_angle=2.0*math.pi, color="red", line_width=2)
...
def two_lines_plot(title, x_label, y_label, line_one_label, line_two_label,
    ...
    plot.line(ns, durations, color='#A6CEE3',
              legend_label=line_one_label, name=line_one_label)
    plot.circle(ns, durations, color='#A6CEE3', size=4)

    plot.line(ray_ns, ray_durations, color='#B2DF8A',
              legend_label=line_two_label, name=line_two_label)
...
def means_stddevs_plot(ns, means, stddevs, title=''):
    tooltips = [
        ...
        ("array size", "$x"),
        ("time", "$y")]
    plot = figure(x_axis_type="log", title=title,
    ...
    plot.rect(x=ns, y=minus_stddevs, width=widths,
              height=0.0001, fill_color="black")
    plot.rect(x=ns, y=plus_stddevs, width=widths,
              height=0.0001, fill_color="black")
